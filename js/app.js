...
function resetForm() {
  ...
  segSet('fTempo', 'up'); segSet('fType', 'original');
...
window.editSong = function(id) {
  ...
  segSet('fTempo', s.tempo); segSet('fType', s.type);
...
function renderSongs() {
  ...
  el.innerHTML = songs.slice().sort((a, b) => a.title.localeCompare(b.title)).map(s => `
    <div class="song-item">
      ...
      ${s.staple ? '<span class="badge staple">${s.type === 'cover' ? '<span class="badge cover">Cover</span>' : ''}#9733; Staple</span>' : ''}
...
$('btnGenerate').addEventListener('click', () => {
  ...
  // Split pool by type
  let poolAll      = songs.filter(s => !mood || s.mood === mood);
  let poolOriginals = poolAll.filter(s => s.type === 'original');
  let poolCovers    = poolAll.filter(s => s.type === 'cover');
  ...
  // Staples are always included, treated like locked songs
  const staples    = songs.filter(s => s.staple);
  const locked     = setlist.filter(x => x.locked && songById(x.songId));
  // Merge staples + locked, avoiding duplicates
  ...
  for (let attempt = 0; attempt < 400; attempt++) {
    ...
    if (nOriginals !== null) {
      ...
      picks.push(...availOriginals.slice(0, need));
    }
    if (nCovers !== null) {
      const need = nCovers - lockedCovers;
      if (need > availCovers.length) continue;
      picks.push(...availCovers.slice(0, need));
    }

    // Add required picks to list first
    for (const s of picks) {
      total += songTime(s, countIntros);
      list.push(s);
    }

    // Fill remaining time with anything not yet used
    const usedIds = new Set([...lockedIds, ...picks.map(s => s.id)]);
    let filler = shuffle(poolAll.filter(s => {
      if (usedIds.has(s.id)) return false;
      if (nOriginals !== null && s.type === 'original') return false; // already have exact count
  ...
  setlist = best.list.map(s => ({ songId: s.id, locked: lockedIds.has(s.id), staple: !!s.staple }));
  saveSetlist(setlist);
  renderSetlist();
