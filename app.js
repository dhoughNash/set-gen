import { loadSongs, upsertSong, deleteSong, loadSetlist, saveSetlist } from './db.js';

/* ================= STATE ================= */
let songs   = [];   // [{id, title, tempo, type, len, intro, mood}]
let setlist = [];   // [{songId, locked}]
let editingId = null;
...
$('btnSave').addEventListener('click', async () => {
  ...
  try {
    ...
    if (editingId) {
      songs = songs.map(s => s.id === editingId ? song : s);
    } else {
      songs.push(song);
    }
    resetForm();
    ...
    status('Error saving song: ' + e.message);
  }
});
...
function resetForm() {
  editingId = null;
  $('fTitle').value = ''; $('fMin').value = ''; $('fSec').value = ''; $('fMood').value = '';
  $('fIntro').value = '0';
  segSet('fTempo', 'up');
...
window.deleteSong = async function(id) {
  ...
  try {
    ...
    status('Error deleting song: ' + e.message);
  }
};
...
function renderSongs() {
  ...
  const el = $('songList');
  if (!songs.length) {
    el.innerHTML = '<div class="empty-note">No songs yet. Add your first one above — it saves automatically.</div>';
    return;
  }
...
function candidatePool() {
  const mix = $('gMix').value, mood = $('gMood').value;
  return songs.filter(s => {
    if (mood && s.mood !== mood) return false;
    if (mix === 'orig'  && s.type !== 'original') return false;
    if (mix === 'cover' && s.type !== 'cover')    return false;
    return true;
  });
}
function shuffle(a) {
...
$('btnGenerate').addEventListener('click', () => {
  ...
  if (!pool.length) { status('No songs match those filters — add songs or loosen the filters.'); return; }

  const locked    = setlist.filter(x => x.locked && songById(x.songId));
  const lockedIds = new Set(locked.map(x => x.songId));
  let best = null, bestDiff = Infinity;

  for (let attempt = 0; attempt < 400; attempt++) {
    const avail = shuffle(pool.filter(s => !lockedIds.has(s.id)).slice());
    let list  = locked.map(x => songById(x.songId));
    let total = list.reduce((t, s) => t + songTime(s, countIntros), 0);

    if ($('gMix').value === 'mostly-orig') {
      avail.sort((a, b) => (a.type === 'cover' ? 1 : 0) - (b.type === 'cover' ? 1 : 0) + (Math.random() - .4));
    }
  ...
  if (!best) { status('Could not build a set — try loosening the tempo rules or the time target.'); return; }
  setlist = best.list.map(s => ({ songId: s.id, locked: lockedIds.has(s.id) }));
  saveSetlist(setlist);
  renderSetlist();
  status('Set built: ' + best.list.length + ' songs, ' + fmt(best.total) + ' against a ' + fmt(target) + ' target. Lock keepers, then regenerate to reshuffle the rest.');
});
...
function arrangeSet(list) {
  ...
  return list; // best-effort fallback
}
...
window.swapRow = function(i) {
  ...
  const pool = candidatePool().filter(s => !used.has(s.id));
